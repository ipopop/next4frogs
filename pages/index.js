...
export default function Home({ talk }) {
	...
	const Increment = useCallback(() => {
		setStep(step + 1);
		(step != 2) & (step != 4) & (step != 6) & (step != 8) & (step != 10) && setSelected(0),
	...
	return isLoading ? (
		<Loader text="Bienvenue sur Next 4 Frogs" status={wordLoading} />
	) : (
		<Layout>
			...
			{step < 12 ? (
				<Quizz step={step} content={textReponses} selected={selected} setSelected={setSelected} />
			) : (
				""
			)}

			<Button step={step} content={textButton} selected={selected} onClick={Increment} />
			<Correction step={step} selected={selected} textCorrection={textCorrection} />
			<Fraugue step={step} />
		</Layout>
	);
}
...
export async function getStaticProps() {
	...
	const res = await fetch("http://next4frogs.sqrl.vercel.app/api/hello");
